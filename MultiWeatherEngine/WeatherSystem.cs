...
{
	// ===== 类型静态参数表 =====
	// v2.0.3 — 尺度全部按比例（SFS 行星=现实 5%、大气=现实 30%）：
	...
	public class TypeSpec
	{
		...
		public double vmaxMs;      // 基准峰值风速（m/s，非台风 category=3 时；台风仅作归一化基准 = PeakWind[5] CAT-4）— v2.2（终审🟡-15）注释修正：62 在 PeakWind 表是索引 5（CAT-4）非 category 3
		...
		// v2.4.5 — 等级上限科学性（用户：似乎所有天气系统都能到最高等级）：maxCat 按类型
	...
	// v2.2.2 — 修复历史错位：Spec 数组元素重排对齐 StormType 枚举（反编译确认 [1..5] 曾错位：
	...
	{
		// v2.0.15 — 云底（现实 AGL → SFS ×0.3）：台风眼壁对流 ~600m、单体/多单体/飑线/MCS
		// 雷暴云底典型 1000-1500m、经典超级单体 LCL ~1000m（HP 更低 LP 更高）。
		new TypeSpec { name = "台风",        rmaxFrac = 0.0079, htopFrac = 0.15, hbaseM = 600,  aspectMin = 0.45, vmaxMs = 62, updraft = 0.32, lifetimeSec = 432000, rotate = 1.0,  downburst = 0.0,  line = 0.0,  gustFront = 0.0,  tornadoHost = false, defaultCat = 4, maxCat = 6, naturalUpTimeSec = 43200, desc = "热带气旋，最大天气系统" },   // v2.4.5 maxCat 6（萨菲尔全表 0-6）；升级 12h/级（现实）
		new TypeSpec { name = "单体",        rmaxFrac = 0.0006,  htopFrac = 0.10, hbaseM = 1200, aspectMin = 0.40, vmaxMs = 18, updraft = 0.50, lifetimeSec = 2700,  rotate = 0.0,  downburst = 0.0, line = 0.0,  gustFront = 0.20, tornadoHost = false, defaultCat = 1, maxCat = 1, naturalUpTimeSec = 0, desc = "普通单体雷暴：发育20+成熟45+消散20≈85min（能量制实际）" },   // v2.4.5 maxCat 1（现实无等级概念，永不升级）；v2.2（终审🟢-5）注释改能量制实际总时长
		new TypeSpec { name = "多单体",      rmaxFrac = 0.0013,  htopFrac = 0.11, hbaseM = 1200, aspectMin = 0.40, vmaxMs = 25, updraft = 0.50, lifetimeSec = 28800, rotate = 0.10, downburst = 0.0, line = 0.0,  gustFront = 0.30, tornadoHost = false, defaultCat = 2, maxCat = 2, naturalUpTimeSec = 0, desc = "多单体风暴：团状，新生单体更替，2-6h" },   // v2.4.5 maxCat 2
		new TypeSpec { name = "超级单体",    rmaxFrac = 0.0019,  htopFrac = 0.12, hbaseM = 1000, aspectMin = 0.50, vmaxMs = 38, updraft = 0.55, lifetimeSec = 28800, rotate = 0.85, downburst = 0.30, line = 0.0,  gustFront = 0.40, tornadoHost = true,  defaultCat = 4, maxCat = 4, naturalUpTimeSec = 0, desc = "超级单体：深厚中气旋，可产龙卷" },   // v2.4.5 maxCat 4
		new TypeSpec { name = "飑线",        rmaxFrac = 0.0031,  htopFrac = 0.12, hbaseM = 1200, aspectMin = 0.40, vmaxMs = 32, updraft = 0.50, lifetimeSec = 64800, rotate = 0.10, downburst = 0.40, line = 0.85, gustFront = 0.95, tornadoHost = true,  defaultCat = 3, maxCat = 4, naturalUpTimeSec = 12960, desc = "飑线：线状+弓形+阵风锋，可产龙卷" },   // v2.4.5 maxCat 4；升级 3.6h/级
		new TypeSpec { name = "中尺度对流系统", rmaxFrac = 0.0065, htopFrac = 0.13, hbaseM = 1200, aspectMin = 0.50, vmaxMs = 30, updraft = 0.50, lifetimeSec = 64800, rotate = 0.30, downburst = 0.30, line = 0.50, gustFront = 0.75, tornadoHost = true,  defaultCat = 4, maxCat = 4, naturalUpTimeSec = 0, desc = "MCS：大尺度，含弓形飑线+多涡旋" },   // v2.4.5 maxCat 4
		// v2.2.1 — 沙尘暴（独立天气系统，用户：沙尘暴应该是独立系统才对）：蒙古气旋/冷锋
		...
		// v2.2.4 — 尺度 0.02→0.006（用户：沙尘暴似乎太大）：原 38km 贴地低层铺天盖地，
		// 砍到 ~11.5km（介于 MCS 8.4km 与台风 15km 之间，符合常见 Haboob 规模）。
		new TypeSpec { name = "沙尘暴",      rmaxFrac = 0.006,  htopFrac = 0.08, hbaseM = 100,  aspectMin = 2.0,  vmaxMs = 28, updraft = 0.15, lifetimeSec = 86400, rotate = 0.25, downburst = 0.0, line = 0.4,  gustFront = 0.80, tornadoHost = false, defaultCat = 2, maxCat = 4, naturalUpTimeSec = 17280, desc = "沙墙推进（Haboob），无降雨" }   // v2.4.5 maxCat 4（GB/T 国标 5 档）；升级 4.8h/级
	};
	...
	// v2.0.10 — 等级名按类型：台风用萨菲尔辛普森（TD/TS/CAT-1~5），
	// 其余系统用通用强度档（解决"单体现 CAT-2"这类台风等级错配）。
	// v2.2.2 — 沙尘暴用气象标准分级（用户：沙尘暴也是有强度的）——
	...
	// v2.0.67 — 台风眼壁半径（Rmax 比例，渲染+风场共用）：按强度分级贴近现实——
	...
	public double Htop;
	public double Hbase;   // v2.0.15 — 云底高度（米 AGL，SFS = 现实×0.3），云体悬浮在空中
	public double Vmax;
	public double Wmax;
	public double drift = 9.0;        // 移动速度（m/s，沿经度方向）
	public double driftAngle;         // 移动方向（弧度，相对经度切线；缓慢随机旋转模拟路径）
	public double moveSpeed;          // v2.3.8 — 风暴实际移动速度（drift × 移速摆动因子，Advance 每帧更新；粒子跟随 CenterVelocity 用它，与风暴中心严格一致）
	// v2.2.0 — 行星大气分级（用户：金星/木星/土星/海王星特有风暴环境）：
	...
	public double age;
	public double lifetime;           // 秒；0 = 持续（台风）（v2.3.0 能量制后死字段，仅存档）
	// v2.4.4.3 — developTime/dissolveTime 死字段已删（v2.3.0 能量制后不参与判定，
	// 发展/消散时长由 developRate/dissolveRate 类型表驱动）
	public int stage;                 // 0 发展 1 成熟 2 消散
	public double intensity = 1.0;    // 强度乘子（随 stage 演化）
	// v2.4.5 — 消散衍生产物（等级上限与消散产物讨论）：台风残余低压——消散期逗点化
	...
	// v2.4.3 — EWRC 眼壁置换（演化讨论高价值 feature）：成熟强台风周期性"先降 20% 风速/
	...
	// v2.3.0 — 能量制生命周期（用户：不写死寿命，改能量制）：energy 0-100 取代固定
	...
	// v2.3.1 — 海温场（用户：冷水会冷死台风；先全面采样海陆分布，再按一套种子生成海温）：
	// 行星海温场 per-planet 一次性生成缓存（360° 每 1° 采样海陆 + 种子低频正弦平滑）。
	// 台风能量制海上按 SST 调制（暖水增强 / 冷水枯竭），HUD 显示海温。
	// v2.3.2 — 冷尾流（用户：台风挖冷水——强风埃克曼抽吸把深层冷水翻上来，暖水层）：
	...
	// v2.2.10 — 消散渐隐兜底（用户：时间加速 2500 万倍也看不到消散，等一两分钟风暴在面板
	// 上直接消失）：Manager 每帧推进 dt = 世界时间差（v2.0.99 钳 30s/帧，覆盖 2000x 内
	...
	public int seed;
	// v2.0.93 — 当前 stage 内演化进度 0→1（HUD 进度条）：发展 f/0.25、成熟 (f-0.25)/0.5、
	// 消散 (f-0.75)/0.25；台风（lifetime=0）无寿命演化返回 0（持续型）。
	// v2.0.94 — 时间制：发展 developTime、成熟至 lifetime−dissolveTime、消散尾段。
	public double StageProgress()
	{
		// v2.3.0 — 能量制：发展 55→80 / 成熟 1.0 / 消散 30→0
		if (stage == 0)
		{
	...
	// v2.0.93 — 生命周期百分比（0-1）。v2.3.0 能量制：= 能量百分比（生命周期剩余）。
	public double LifeFrac()
	{
		return Clamp01(energy / 100.0);
	}

	// v2.0.2 — 附属现象：龙卷（宿主成熟期产生，漏斗形渲染）与下击暴流（局部下沉+辐散）。
	// v2.0.85 — 附属现象多实例（用户：随机合适位置生成 + 数量限制解除）：龙卷/下暴从单标量
	...
	public class FxInst
	{
		...
		public int variant;        // v2.4.1 — 龙卷类型（0=标准 1=楔形 2=绳状 3=水龙卷 4=陆龙卷，自动派生见 Advance）
		// v2.4.4 — 类型扩展：5=多涡旋（主漏斗内 2 子涡快速绕转）、6=卫星龙卷（外侧 1 小
	...
	// v2.1.2 — 新增附属现象：阵风锋（下暴出流的地面前沿锋面，弧状云墙+强出流）与
	...
	public double gustFrontStrength;  // v2.1.2 — 至少一个阵风锋时的最大强度
	// v2.1.2 — 台风海陆检测（地理位置可行性落地）：SFS 是 2D 行星，无经纬度概念，
	...
	// v2.0.98 — 强度平滑过渡（用户：强度切换搞粒子过渡）：F8/自然升级设置 vmaxTarget，
	// 实际风场/渲染用 vmaxDisplay 每帧向 target 逼近（~3 秒）——切强度风场平滑爬升不跳变。
	public double vmaxDisplay;
	public double vmaxTarget;
	public double vmaxTargetBase;   // v2.3.6 — 当前 category 档位基准（SetCategory 更新，能量驱动风速用）
	public double wmaxDisplay;
	// v2.0.98 — 自然发展强度进度（用户：所有系统自然发展强度也需进度条、满了到下一等级）：
	// 成熟期自然累积，满 Spec.naturalUpTimeSec 后 category+1（钳 maxCat）并重算目标强度。
	// v2.4.5 — naturalUpTime 字段已删（升级节奏入 Spec：台风 12h/飑线 3.6h/沙尘暴 4.8h/级，
	// ×cL 大气分级——原恒 300s 与寿命现实化比例荒谬）。
	public double naturalProgress;      // 0~1 进度条
	// v2.0.98 — 系统类型转变（用户：系统可能向另一个系统转变，粒子过渡动画原基础上转变）：
	...
	// v2.0.98 — 转变完成后通知渲染端重建粒子（按新类型分布）。
	public bool RebuildPuffsFlag;

	// v2.0.51 — 合并动画状态：被吞方/参与方启动 mergeAnimT(0→1，3 秒) 后渲染中心向
	...
	// v2.2.6 — 消散渐隐因子（用户：粒子消失可能是消散机制问题）：原消散期（stage=2）粒子
	...
	public float DissolveFade()
	{
		if (dissolving)
		{
			// v2.2.10 — 帧驱动兜底：能量枯竭后的 60 帧渐隐（时间加速下也可见，
			// 不再被 dt 压缩成零点几秒的"啪"消失）。
			return Mathf.Clamp01(dissolveCountdown / 300f);   // v2.4.3 — 演化：兜底 60→300 帧（5s 渐隐，别让消散最后 1s 啪消失）
		}
		if (stage != 2)
		{
			return 1f;
		}
		// v2.3.0 — 能量制：消散期能量 30→0 线性淡出（随崩溃渐隐，任何倍率可见）
		return (float)Math.Min(1.0, Math.Max(0.0, energy / 30.0));
	}

	// v2.0.51 — 合并动画渐隐因子：被吞方（mode 0/1）动画期间整体淡出（×0.9 斜率），
	...
	// v2.0.51 — 合并动画渲染中心：行星坐标（被吞方插值向目标/中点）。
	public Double2 MergedStormC()
	{
		...
		if (mergeAnimT >= 0.0 && planet != null)
		{
			...
			if (mergeMode == 2)
			{
				...
				// v2.4.4 — Fujiwhara 互旋（演化讨论：SFS 2D 单自由度完整互旋几何不可做，
	...
	// v2.0.77 — 11 区索引（按 sR 分段，与 StormRenderer.windZoneNames 对应）。
	// v2.0.79 — 分段重排（用户确认 11 区是否覆盖整个台风——旧分段只到 ±2.2Rmax 而风场
	...
	// v2.0.83 — 蒲福风级（中国标准下限，m/s）：级0=静风 ... 级7=13.9(疾风) 级8=17.2 级9=20.8
	...
	// v2.0.83 — 基础风力轮廓（无 11 区系数）：|sR|≤0.5 风眼 0.2→0.5、≤1.0 升到眼壁峰 1.0、
	// 之外衰减到 0.2（v2.0.76 对称双峰）。静态，供 WindCircleRo/线性过渡复用。
	...
	// v2.1.1 — 性能：WindCircleRo ×4 的结果只依赖系统状态（vmaxDisplay×intensity、Rmax、
	...
	// v2.0.83 — 风圈半径（Rmax 单位）：风速 ≥ levelMin(m/s) 的最远半径。从外向内扫
	// BaseProfile×gain ≥ levelMin/Vmax 的第一个位置。返回 0 表示无该等级风圈（弱风暴够不到）。
	// v2.0.84 — baseOnly=true：忽略 11 区系数（纯 BaseProfile）——风圈保底用稳定骨架，
	// 系数被调 0 时风圈不塌（否则保底随系数消失，失去兜底意义）；HUD/黑框显示用默认（含系数）。
	// v2.0.97 — 扫描目标含 edge 衰减（×exp(−(a/2.5)²)，与 SampleComponents 的 edge 基准
	...
	// v2.0.13 — 返回 bool：仅 tornadoHost 类型（超级单体/飑线/MCS）可产生龙卷。
	// v2.0.54 — 手动添加的附属现象常驻：manual=true 时不自然衰减（只响应显式解散）。
	// v2.0.85 — 多实例 + 随机位置：每次添加生成新实例，位置在风暴内随机（沿 s 轴）。
	public bool manualTornado;
	public bool manualDownburst;

	// v2.0.85 — 确定性随机偏移（种子混合实例序号，同风暴同位置稳定）。
	// v2.0.86 — attempt 参数：防重叠重试时扰动种子（否则重试生成相同位置死循环）。
	...
	// v2.0.86 — 防重叠：与已有实例（同类 minGap、异类 crossGap，Rmax 单位）间距检查。
	...
	// v2.0.86 — 随机位置 + 防重叠（重试 12 次找不重叠落点；isTornado 决定同类间距）。
	...
	// v2.4.4 — 龙卷类型自动派生扩展（类型调研方案二期）：海洋→水龙卷（3）；超单强龙卷
	...
	private int DeriveTornadoVariant(FxInst fx)
	{
		...
		// v2.2（终审🟢-7）— Ocean 判定移到超单强龙卷之后：原 Ocean 优先导致海上超单强
	...
	// v2.0.93 — 下暴宿主白名单（用户：禁用台风/单体/多单体生成下击暴流——现实下暴主要
	...
	// v2.0.93 — 返回 bool（宿主不允许时拒绝并返回 false，F6 菜单可提示）。
	public bool AddDownburst()
	{
		...
		FxInst fx = new FxInst();
		// v2.0.98 — 强度跟母体：0.5 + category/6（cat3=1.0 基准）。
		fx.strength = 0.5 + category / 6.0;
		fx.phase = 0.0;
		fx.dissolving = false;
		fx.manual = true;
		RandomPosAvoid(fx, 0.6, false);         // v2.0.86 — 随机合适位置 + 防重叠
		downbursts.Add(fx);
	...
	// v2.1.2 — 阵风锋：下击暴流出流（冷池）在地面的推进前沿，气象上阵风锋出现在任何能
	// 产生下暴的系统（超级单体 RFD / 飑线弓形前沿 / MCS 出流边界）——白名单与下暴一致。
	public bool CanGustFront()
	{
		// v2.2.1 — 沙尘暴也允许挂阵风锋（Haboob 沙墙前沿 = 阵风锋的沙漠形态）
	...
	// v2.1.2 — 闪电风暴：中气旋/眼壁内密集闪电。气象上所有深对流系统（含台风眼壁与
	...
	// v2.0.18 — 清除改为"渐消"：强度保留，由 Advance 以 ~3 秒速度衰减（消散动画）。
	// v2.0.85 — 多实例：全部实例置 dissolving。
	public void ClearPhenomena()
	{
		...
		foreach (FxInst fx in gustFronts)        // v2.1.2
		{
			fx.dissolving = fx.strength > 0.001;
		}
		foreach (FxInst fx in lightningBursts)   // v2.1.2
		{
			fx.dissolving = fx.strength > 0.001;
		}
	...
	// v2.0.86 — 附属现象合并（同类大吞小）：龙卷×龙卷 / 下暴×下暴 sOff 间距 < 0.4Rmax
	...
	private void MergeFx()
	{
			...
			for (int i = 0; i < tornadoes.Count; i++)
			{
				for (int j = i + 1; j < tornadoes.Count; j++)
				{
					...
					if (Math.Abs(a.sOff - b.sOff) * Rmax < 0.4 * Rmax)
					{
						...
						{
							a.sOff = mid;
							a.strength = Math.Min(1.4, a.strength + b.strength * 0.25);   // v2.2 吸收上限 2.0→1.4（终审🟡-5：原 90×2.0×1.82≈328 m/s 超 EF5）
							tornadoes.RemoveAt(j);
						}
						else
						{
							b.sOff = mid;
							b.strength = Math.Min(1.4, b.strength + a.strength * 0.25);   // v2.2 吸收上限 2.0→1.4（终审🟡-5）
		...
		// v2.1.2 — 阵风锋同类合并（与龙卷/下暴同构）：间距 < 0.5Rmax 大吞小。
	...
	public void Configure(StormType t, Planet p, double angle, int catBoost)
	{
		...
		planet = p;
		centerAngle = angle;
		// v2.4.5 — 初始档按类型 maxCat 钳（防 defaultCat+catBoost 超自然上限；F8 手动可
		...
		// v2.0.63 — 尺寸按现实 ×30%（用户：所有天气现象长宽度按现实 30%，忽略地表是现实 5%，
		// 压迫感十足）：原 Rmax=行星半径×rmaxFrac 在 5% 行星上把台风缩成 2.4km 没压迫感。
		// 改 Rmax = 6371km×rmaxFrac×0.3（现实核心半径×30%，无视行星大小）；钳 [100m, 0.3×行星半径]。
		// v2.2.0 — Rmax 赋值移至 atmoClass 分级系数（cR）声明之后（见下方）。
		// v2.0.6 — 高宽比下限：SFS 大气相对行星厚（30% vs 5%），纯比例会让小系统变成
		...
		// v2.2.0 — 行星大气分级（SFS Terra 大气 ~60km 为基准，任何行星 mod 自动生效）：
		...
		Rmax = 6371000.0 * spec.rmaxFrac * 0.3 * cR;   // v2.2.0 × 大气分级（在 cR 声明后）
		// v2.0.63 — Htop 同改为现实×30%（100km 大气基准 × htopFrac × 0.3），钳制不超大气顶。
		Htop = 100000.0 * spec.htopFrac * 0.3 * cH;   // v2.2.0 × 大气分级
		if (Htop < 500.0)
		{
			Htop = 500.0;
		}
		...
		// v2.0.15 — 云底：现实 LCL 云底 × 0.3（SFS 大气 30%），钳 [30m, Htop×0.4]。
		...
		Router = Rmax * 9.0;
		// v2.0.51 — 台风强度分级修正（用户：连热带低压都 11 级风）：原通用公式 0.5+cat/6
		// 在 cat=0(TD) 时给 62×0.5=31 m/s（11 级）离谱。台风用真实分级曲线。
		// v2.2（终审🟡-1）— 台风 catF 统一用 Category.PeakWind 萨菲尔表（与 SetCategory
		// 单一口径，v2.4.3"三套口径根治"只改了 SetCategory 漏了 Configure——原中国标准
		...
		Vmax = spec.vmaxMs * catF * cV;   // v2.2.0 × 大气分级（巨行星强对流高速）
		...
		{
			// v2.2.0 — 巨行星风暴寿命 ×30（台风 3 天 → 90 天，大红斑类；非永久——用户反对 0=持续）
		...
		age = 0.0;
		// v2.3.0 — 能量制：所有系统能量 55 起步（发展），stage 由 energy 驱动（无 lifetime
		// 特例——台风也走完整发展→成熟→消散，不再"持续成熟"）。
		energy = 55.0;
		stage = 0;
		intensity = 0.4;
		// v2.0.94 — 发展阶段 60-120s 随机（用户：发展太慢——原发展期 = 寿命 25%，单体
		// ~9 分钟太久）；消散期 = 寿命后 10%（最小 30s）。
		// v2.4.4.3 — developTime/dissolveTime 死字段赋值已删（v2.2.10 消散窗口现实化是
		// lifetime 时间制时代的机制，v2.3.0 能量制已由 stage==2 帧化 + dissolveRate 接管；
		// 5 分钟窗口兜底随字段退役——用户：这些保留也可以去了）。
		// v2.0.26 — drift 按 SFS 行星尺度换算（×0.35）：原现实速度 9-20 m/s 在 5% 大小行星上
		// 绕行星快 20 倍，龙卷/风暴视觉移动过快。换算后超单 3.7-5.8 m/s、台风 5-7 m/s，
		// 接近真实龙卷平移（4.5-9 m/s）在 SFS 上的观感。
		drift = (5.0 + spec.vmaxMs * 0.15 + 6.0 * ((double)(seed % 97) / 97.0)) * 0.35 * cD * ((type == StormType.DustStorm) ? 3.0 : 1.0);   // v2.2.0 × 大气分级；v2.2.1 沙尘暴随大风带移动快（×3）
		driftAngle = (double)(seed % 6283) / 1000.0;
		commaDir = ((seed % 2) == 0) ? 1.0 : -1.0;   // v2.4.5 — 残余低压逗点方向（种子确定，±1 随机）
		moveSpeed = drift;   // v2.3.8 — 实际移动速度初始 = drift（Advance 首帧起叠加移速摆动）
		active = true;
		// v2.0.98 — 强度平滑：显示值 = 目标值（初始无过渡）。
		vmaxDisplay = Vmax;
		vmaxTarget = Vmax;
	...
	public void SetCategory(int cat)
	{
		...
		// v2.4.3 — 待办5 🟡-8 三套口径根治：台风 catF 直接对齐 Category.PeakWind（萨菲尔表
		...
		double catF = (type == StormType.Typhoon) ? (Category.PeakWind[category] / 62.0) : (0.5 + category / 6.0);
		// v2.3.6 — 审查🔴-2：补乘大气分级 cV（原漏乘 → 巨行星/金星系统升级后风速掉回地球基准）
	...
	// v2.2（终审🟡-14）— 眼壁风补 edge 衰减：原 vmaxDisplay×1.2（名义眼壁增益）但风场
	...
	// v2.0.98 — 附属现象强度跟母体：龙卷/下击暴流 strength = 0.5 + category/6（cat3=1.0）。
	private void SyncFxStrength()
	{
		...
		foreach (FxInst fx in gustFronts)        // v2.1.2 — 阵风锋强度跟母体
		{
			fx.strength = s;
		}
		foreach (FxInst fx in lightningBursts)   // v2.1.2 — 闪电风暴强度跟母体
		{
			fx.strength = s;
		}
		tornadoStrength = (tornadoes.Count > 0) ? s : 0.0;
		downburstStrength = (downbursts.Count > 0) ? s : 0.0;
		gustFrontStrength = (gustFronts.Count > 0) ? s : 0.0;   // v2.1.2
	}

	// v2.0.98 — 类型转变（原基础上转变，粒子过渡动画）：3 秒淡出淡入，完成后按新类型重配尺度。
	...
	private void ApplyTypeScale()
	{
		...
		Router = Rmax * 9.0;
		rmaxBase = Rmax;
		// v2.0.99 — 修复：vmaxBase 原来取旧类型 Vmax（SetCategory 之前），合并上限基准错。
		// v2.4.5 — 顾问审查抓的潜伏 bug：类型转变后 SetCategory 只重算 vmaxTargetBase/
	...
	public void Advance(double dt)
	{
		...
		// v2.0.99 — 钳制 5s→30s：原 5s 上限在时间加速（warp）下掉队——1000x 时每帧
		...
		// v2.0.86 — 附属现象合并：同类（龙卷×龙卷 / 下暴×下暴）sOff 间距 < 0.4Rmax → 大吞小
		// （保留强度大者，位置取强度加权中点；被吞者直接移除——与天气系统合并同风格）。
		MergeFx();
		// v2.2.10 — 消散窗口帧化（用户：任何时间倍率都看不到"消散"，等一两分钟风暴在面板
		// 上直接消失）：时间加速下 dt 钳 30s/帧（v2.0.99）会把消散压成零点几秒。
		// v2.3.0 — 能量制：stage==2（消散崩溃）时钳 0.5s/帧，能量归零过程可视化
		// （DissolveFade = energy/30 线性淡出，防大 dt 一次归零跳过渐隐）。
		// v2.2（终审🟡-7）— 消散帧化钳 0.5s→6s：原 0.5s/帧 = 恒 30 游戏秒/现实秒，台风 12h
		...
		age += effDt;
		// v2.1.2 — 台风海陆检测（节流 8s，GetTerrainHeightAtAngle 有数组分配不能每帧调）+ 登陆衰减。
		...
		{
			lastLandCheck = age;
			// v2.1.3 — 地形类型统一判定（TerrainAt 内部：高度场分海陆 + 纹理色细分沙漠/绿地/冰/岩）
			// v2.1.5 — 地形检测对所有风暴生效（用户：地形对所有风暴生效）：terrainKind/overLand
		...
		{
			// v2.3.0 — 能量制替代 age×3 硬编码加速：地形输入因子（terrainE 陆地降）让
			// 登陆台风净耗散率升 → 能量快速枯竭 → 自然走消散流程（真实气象：登陆后
			// 水汽切断 + 地面摩擦，12-24h 内消散）。
			// v2.3.7 — 审查二轮：风速摩擦衰减改为**乘性**并移到能量公式之后（stage 段）
			// 执行——原减法在能量公式前被 vmaxTargetBase×能量因子覆盖成死代码。此处仅
			// 保留 overLand 状态（能量段用）。
		}
		// v2.0.98 — 强度平滑逼近（切强度/自然升级时风场 ~3 秒爬升，粒子过渡不跳变）。
		...
		if (mergeAnimT >= 0.0)
		{
			// v2.4.3 — 演化：合并动画 3s→12s（Fujiwhara 互旋逼近的"互旋张力"需要更长
			// 的逼近过程，审查讨论：3s 秒合体太突兀，10-15s 有重组感）。
			// v2.4.4.1 — 合并动画帧化（用户：时间加速下是否可控）：原 dt/12 是游戏时间
		...
		// v2.3.0 — 能量制生命周期（用户：不写死寿命，改能量制——任何倍率都要能看到消散）：
		...
		// v2.2（终审🟢-4）— Ice 0.30→0.18、沙漠（非沙尘）0.22→0.28：冰原几乎无深对流
		...
		// v2.3.7 — 审查二轮：沙尘暴离开沙漠 → 0.35（无沙源衰竭，~42min 耗尽；不触发硬
		// 失败 → F6 海上召唤不消失，保留玩家自由）。顺带修掉 🟢-15"出海反而活更久"。
		else if (type == StormType.DustStorm) terrainE = 0.35;
		// v2.3.1 — 台风海上按海温调制（用户：冷水会冷死台风——气象铁律，SST 掉 1-2°C
		...
		if (type == StormType.Typhoon && terrainKind == TerrainKind.Ocean)
		{
			...
			// v2.4.4 — 🟡-4 数值层（审查：金星 33°C/巨行星 30°C 是地球值硬套——金星表面
			...
			DigColdWater();   // v2.3.2 — 冷尾流：台风挖冷水（本帧 sstF 用挖冷前值，下帧见挖后值，平滑）
		}
		// 类型成熟期净耗散（能量/游戏秒，海上参考；÷terrainE → 陆地更快枯竭）
		// v2.4.4.2 — 寿命全部回归现实尺度（用户：一切寿命和现实相同，至于长忽略——等得起，
		// 时间加速就是为等待准备的）：退役 v2.3.0"缩短到可等尺度"与 v2.4.3"拉大差距"的
		// 游戏化标定，按真实气象寿命（成熟期 80→30 掉 50 能量 = 该时长）：
		//   台风 5 天（432000s）  单体 45 分钟（2700s）  多单体 8 小时（28800s）
		//   超单 8 小时（28800s） 飑线 18 小时（64800s） MCS 18 小时（64800s）
		//   沙尘暴 24 小时（86400s）
		// 发展期保持快（v2.0.94 用户：发展太慢——2.5 分钟成型），消散崩溃 20× 相对语义。
		// v2.2（终审🔴-1）— 大气分级 cLnow 提前定义（三速率表共用）：developRate/dissolveRate
		...
		// v2.4.4.3 — 发展期/消散期现实化（用户：发展期 2.5 分钟成型、消散 20× 相对语义这些
		// 保留也可以去了，发展期交给时间加速）：退役 v2.0.94 发展 60-120s 快进与 20× 消散
		...
		developRate /= cLnow;   // v2.2（终审🔴-1）— 大气分级同步（巨行星发育 ×30 放慢）
		...
		dissolveRate /= cLnow;   // v2.2（终审🔴-1）— 大气分级同步（巨行星消散 ×30 放慢，三阶段统一）
		// v2.4.4.2 — 下限 0.0005→0.00005：台风现实 5 天 = 0.000116/s，被旧下限钳成 28h
		// （"可等尺度"时代的钳值，现实化后失效）——下限只是防净耗散为 0 的保险，须低于
		// 最慢类型（台风海上 0.000116）。
		netPerSec = Math.Max(netPerSec / ((type == StormType.Typhoon && terrainKind == TerrainKind.Ocean) ? (terrainE * terrainE) : terrainE), 0.00005);   // v2.3.7 海上台风 ÷sstF²：24°C 骤降可见更快（冷水冷死台风）
		// v2.4.4.2 — 下限 0.00002→0.000002：巨行星台风 5 天×30 = 0.0000039/s，旧下限钳成 29 天
		netPerSec = Math.Max(netPerSec / cLnow, 0.000002);

		// v2.4.4 — 修复 stage 边界振荡（用户：79↔80% 反复发展、成熟横跳）：原 stage 每帧
		...
		if (stage == 0)
		{
			// v2.3.6 — 审查🔴-1：发展期能量受环境反馈（原恒 +0.167/s 不看海温/地形，
			// "冷水区台风发育不良"落空——26.5°C 阈值只作用成熟期）。环境因子 envF =
			// terrainE（台风海上已含 sstF）：暖海 1.0 → 2.5min 成熟；绿地 0.55 →
			// ~4.5min；冷水（sstF<0.35，≤20°C）→ **发育失败**：能量下跌 → 自然消散
			// （气象：热带气旋生成对 SST 最敏感，冷水里长不大）。
			// v2.3.7 — 审查二轮：硬失败仅限台风（海上 sst<24°C 或陆地——TC 陆生不可能）
			// 与沙尘暴（无沙源）；对流系统（单体/多单体/超单/飑线/MCS）冰原/沙漠走慢发育
			// 不判死（大气不稳定驱动，与地表弱相关；极地确有弱雷暴）。24°C 失败线与
			// HUD 三档（≥26.5 增强/≥24 临界/<24 冷水）自洽。
			...
			{
				// v2.4.4.3 — 发展速率现实化（用户：发展期交给时间加速）：原恒 0.167/s
				// （2.5 分钟成型——v2.0.94 游戏化快进），改按类型现实发展时长（台风 2.5 天
		...
		else if (stage == 1)
		{
			...
			if (type == StormType.Typhoon && !overLand && energy > 70.0)
			{
				if (ewrcT < 0.0)
				{
					...
					{
						ewrcT = 0.0;
						ewrcCooldown = 86400.0;   // v2.4.4.2 — 现实化：置换后 1 天冷却（真实 EWRC 周期 1-3 天；台风 5 天寿命 ~5 次置换）
					}
				}
			}
			...
			// v2.4.4.1 — EWRC 置换过程帧化（用户：时间加速下是否可控）：原 effDt/180
			// 是游戏时间——加速 100x 时 3 分钟置换被压成 ~2 帧（0.03s 现实），"先弱
			// 后强、眼先糊后清"肉眼不可见（与 v2.2.10 消散同样的问题）。帧化：每帧
			...
			// v2.3.6 — 审查🔴-3：能量驱动风速渐进削弱（原挖冷/冷水/登陆只掉能量条不掉
			...
			if (ewrcT >= 0.0)
			{
				...
				// v2.4.4.1 — 用帧化 ewrcStep（与 ewrcT 同步，防能量降/回补与置换相位错位）
				// v2.2（终审🟡-2）— 升段 10→14：原降 12/升 10 净 -5.4/周期且是成熟期能量主耗散
		...
		{
			// v2.4.4.3 — 消散速率现实化（用户：消散 20× 相对语义可以去了）：原 netPerSec×20
			// （相对成熟快 20 倍——不同类型比例失衡：台风 3.6h 合理但单体 81s 太快）。改按
			// 类型独立现实时长（台风 12h/单体 20min/…），÷terrainE → 登陆/沙漠摩擦加速
			// 消散（台风登陆 12h → 岩石 5.4h/沙漠 2.7h，物理正确）。
			// v2.4.5 — 台风残余低压（消散产物讨论）：消散期逗点化（commaK 0→1，energy
		...
		// v2.4.5 — 泥雨 muddyFactor（消散产物讨论：沙尘暴消散沙尘沉降，与降雨系统相遇 →
		...
		// v2.3.7 — 审查二轮：land decay 改乘性放能量公式后（原减法在能量公式之前执行，
		...
		if (energy <= 0.0)
		{
			// 能量枯竭 → dissolving 60 帧渐隐兜底（渲染淡出 + HUD 显示消散，完成才移除）
				...
				{
					dissolving = true;
					dissolveCountdown = 300f;   // v2.4.3 — 演化：60→300 帧（5 秒现实渐隐，任何倍率可见不"啪"消失）
				}
			else
			{
		...
		// v2.3.7 — 审查二轮（🟡-11）：SFS 2D 行星表面是圆周、只有 1 个自由度，路径弯曲
		...
		// v2.3.8 — 修复：moveSpeed 存字段并供 CenterVelocity() 使用——风暴实际移动速度
		...
		// v2.2（专项 B 绕圈真 bug）— 移动用 effDt 而非 dt：消散期（stage==2）能量衰减被
		...
		// v2.0.2 — 附属现象自然演化（成熟期：超级单体/飑线/MCS 概率产龙卷；所有强系统概率下击暴流）
		// v2.0.18 — 消散动画：dissolving 时 ~3 秒衰减（ClearPhenomena 触发），否则自然缓慢衰减。
		TypeSpec spec = Spec[(int)type];
		if (stage == 1)
		{
			// v2.0.98 — 自然发展强度进度（用户：自然发展强度也需进度条、满了到下一等级）：
			// 成熟期自然累积 naturalUpTime（~5 分钟），满 → 下一等级（钳 6）→ 目标强度
			// 平滑过渡（粒子过渡动画）；手动 F8 切档时 naturalProgress 由 Manager 重置。
			// v2.2.5 — 满级钳制（用户：达到最高等级后进度条还在动，1000%都可以）：
			// category=6 时不再累积，进度条停满 100%（原条件 category<6 才重置，满级后
			// naturalProgress 继续涨 → HUD 显示 1000%+）。
			// v2.4.5 — 等级上限科学性（用户：所有天气系统都能到最高等级不科学）：判据
			...
			int maxC = TyphoonConfig.I.limitMaxCategory ? Math.Max(1, Math.Min(6, spec.maxCat)) : 6;   // v2.4.5 关=chaos 全 6 级
			// v2.2（终审🟡-4）— naturalUpTimeSec=0 的类型（maxCat==defaultCat，本不需升级）
			// 跳过累积——原 dt/max(0,1)=dt 导致类型转变后（如单体→多单体 category 保持 1）
			// 1 游戏秒内瞬时跳级回 defaultCat。
			if (spec.naturalUpTimeSec > 0.0 && category < maxC)
			{
				// v2.2（fix-checker ⚠️-1）— 升级节奏 ÷cLnow（原 *cLnow 方向反：巨行星升级
				...
				if (naturalProgress >= 1.0)
				{
					...
					energy = Math.Min(100.0, energy + Math.Min(12.0, 450.0 * netPerSec));   // v2.3.7 审查二轮：升级回血 min(12, 450×netPerSec)——净回血不超过环境耗散（冷水 0.0185×450=8.3），不再掩盖冷水负反馈
				}
			}
			else
			{
			...
			// v2.0.98 — 自然类型转变（用户：系统可能向另一个系统转变）：成熟期低概率向
			// 更强相关类型转变（单体→多单体→超级单体→MCS、飑线→MCS），台风不转；
			// 3 秒粒子过渡动画（渲染端云淡出淡入），完成后按新类型重配尺度。
			// v2.2.1 — 沙尘暴不参与类型转变（干燥系统，不会变成雷暴）
			...
			if (spec.tornadoHost)
			{
				// v2.0.85 — 多实例（数量限制解除，上限 4 防爆渲染）：遍历衰减（manual 常驻
				// 跳过）、自然触发随机位置添加；v2.0.51 自然寿命 150s、v2.0.18 解散 2.5s。
				...
				{
					FxInst fx = new FxInst();
					fx.strength = 0.5 + category / 6.0;   // v2.0.98 — 强度跟母体
					fx.phase = 0.0;
					fx.dissolving = false;
					fx.manual = false;                  // v2.0.54 — 自然触发按寿命衰减
					fx.variant = DeriveTornadoVariant(fx);   // v2.4.4 — 类型自动派生（同 AddTornado，含多涡/卫星/gustnado/陆龙卷）
					RandomPosAvoid(fx, 0.9, true);   // v2.0.86 — 随机 + 防重叠
					tornadoes.Add(fx);
				}
				tornadoStrength = 0.0;
				foreach (FxInst fx in tornadoes)
			...
			// v2.0.85 — 下暴多实例同理（上限 4，自然寿命 240s）。
			...
			if (downbursts.Count < 4 && CanDownburst() && (seed * 2654435761u % 10000) < dt * 30.0)
			{
				...
				fx.dissolving = false;
				fx.manual = false;
				RandomPosAvoid(fx, 0.6, false);   // v2.0.86 — 随机 + 防重叠
				downbursts.Add(fx);
			}
			downburstStrength = 0.0;
			foreach (FxInst fx in downbursts)
			...
			dissolvingDownburst = downbursts.Count > 0;
			// v2.1.2 — 阵风锋自然演化（成熟期，上限 3）：衰减/相位同下暴，寿命 300s。
			...
			// v2.1.2 — 闪电风暴自然演化（成熟期，上限 2）：寿命 400s。闪电风暴不产生风，
		...
		{
			// v2.0.54 — 手动添加的常驻：非成熟期也不衰减（仅显式解散时衰减）。多实例遍历。
			for (int ti = tornadoes.Count - 1; ti >= 0; ti--)
			{
				FxInst fx = tornadoes[ti];
				if (fx.dissolving || !fx.manual)
	...
	// v2.2.8 — includeDrift：false 时返回"相对风暴的风"（不含 drift 平流分量）。
	...
	// v2.2.8 — 风暴中心移动速度（drift 沿经度正方向切向，与 ToStormFrame 的 s 同向）：
	...
	public Double2 CenterVelocity()
	{
		...
		return new Double2(0.0 - Math.Sin(centerAngle), Math.Cos(centerAngle)) * moveSpeed;   // v2.3.8 — 跟随实际移动速度（moveSpeed 摆动）而非固定 drift
	}
	...
	public Double2 SampleWindLocal(double s, double h, Double2 globalPos, bool includeDrift = true)
	{
		...
		// v2.0.23 — 附属现象水平矢量风（基于玩家相对风暴中心的水平位置）：
		...
		if (active && planet != null)
		{
			...
			if (gustFrontStrength > 0.05)   // v2.1.2 — 阵风锋：地面出流前沿强风
			{
	...
	// v2.0.23 — 风暴中心的行星坐标（centerAngle 处、地面高度），供矢量风计算相对位置。
	...
	// v2.0.85 — 附属现象实例中心（风暴中心 + 切向偏移 sOff×Rmax，沿经度方向）。
	...
	// v2.3.1 — 行星海温场（用户：冷水会冷死台风；先全面采样海陆分布，再按一套种子生成海温）：
	...
	public static float[] GetSstField(Planet pl, int atmoClass)
	{
		...
		sstCache[pl] = sst;
		sstOrigCache[pl] = (float[])sst.Clone();   // v2.3.2 — 原始场备份（冷尾流恢复目标）
		return sst;
	}
	...
	// v2.3.2 — 台风挖冷水（冷尾流，用户：台风搅动把冷水翻上来，暖水层）：台风强风
	// 埃克曼抽吸 → 深层冷水上涌 → 中心海域 SST 下降（挖冷）→ 能量制 sstF 降 → 台风
	// 自我削弱（负反馈闭环——现实台风停滞时冷尾流会"咬死"自己）。暖水层（混合层）
	// 限制可挖深度：挖穿后无冷水可挖。节流 1s（age 基准）；海温缓慢向原始值恢复。
	// v2.3.3 — 按真实资料校准（Nature 2025 / J. Operational Oceanography 2023 /
	// 南海 MLD 研究）：① 冷尾流 SST 降幅观测上限 ~4.76°C → 暖水层可挖 3-6°C；
	// ② 慢速台风（<2m/s）冷尾迹在中心、快速台风（>6m/s）在路径右侧（北半球右象限
	// 近惯性混合更强）→ 挖冷中心向 drift 方向偏移 1° 近似右侧偏差；
	// ③ 恢复现实 12-30 天 → 恢复速率减半（游戏化慢恢复，冷尾流可观察持续）。
	private void DigColdWater()
	{
		// v2.2（终审🟢-3）— gate atmoClass==0：巨行星无"海"、金星无液态水，挖冷是无用计算
		...
		int bias = (drift >= 6.0) ? 1 : 0;   // v2.3.7 审查🟡-5：快速台风（drift≥6 m/s）冷尾迹偏路径右侧（近惯性混合），慢速挖中心（Ekman 泵主导）；SFS 2D 无科氏力，此为游戏化偏移
		cIdx = (cIdx + bias + 360) % 360;
		// v2.3.4 — 搅动强度与台风强度关系校准（用户：这块没搜——Nature 2025 原文：
		// 风应力对风速呈二次方依赖（quadratic dependence），垂直混合/埃克曼抽吸由
		// 风应力驱动 → 冷却速率按 vmax² 缩放而非线性）。
		// v2.2（终审🟡-3/13）— 冷尾流恢复与挖深校准（Dare & McBride 2011：冷尾流
		...
		for (int d = -3; d <= 3; d++)
		{
			...
			f[idx] += (orig[idx] - f[idx]) * 0.00001f;   // v2.2 恢复校准（时间常数 ~1.16 天，现实 5-20 天加速版）
		}
	}

	// v2.2.1 — 静态版（自然生成判断用，无需 WeatherSystem 实例）：行星 + 位置 → 地形类型。
	...
	// v2.0.23 — 龙卷水平旋转矢量：方向 = 玩家相对龙卷中心的水平切向（绕垂直轴旋转），
	// 大小 = 90m/s EF5 基准 × rotCore（核内刚体平滑）× funnel（地面最强）。
	// v2.0.85 — 多实例遍历（数量限制解除）：每个龙卷独立中心（sOff 偏移）与强度累加。
	private Double2 TornadoHorizontal(double s, double h, Double2 globalPos)
	{
		...
		for (int ti = 0; ti < tornadoes.Count; ti++)
		{
			...
			double edge = Math.Exp(0.0 - Pow2(num / (Rmax * 2.5)));   // v2.0.93 — 收紧（原 Router）
	...
	// v2.0.23 — 下击暴流水平辐散矢量：方向 = 玩家相对中心径向向外，大小 = 出流风速
	// （dro 0.4-1.6 起效、近地面最强）。v2.0.85 — 多实例遍历（独立中心/强度累加）。
	...
	// v2.1.2 — 阵风锋水平出流矢量：锋面（sOff 处）附近地面强出流，方向沿 +s（行星切向，
	...
	// v2.0.53 — 公开诊断：玩家位置处下击暴流出流速度（m/s），用于 HUD 调试确认生效。
	...
	// v2.2.8 — includeDrift：false（粒子相对风）时跳过 drift 平流分量。Probe/玩家测风
	...
	public void SampleComponents(double s, double h, out double u, out double w, bool includeDrift = true)
	{
		...
		// v2.0.93 — 风暴范围外无风（用户：不在风暴范围仍有风影响，所有风暴都是）：
		...
		double edge = Math.Exp(0.0 - Pow2(numW / (Rmax * 2.5)));   // v2.0.93 — 收紧衰减
		double hDecay = Math.Exp((0.0 - ht) / 0.14);
		double gust = spec.gustFront;

		// v2.0.2 — 附属下击暴流：局部强下沉 + 低层辐散。
		// v2.0.23 — 水平辐散从标量 u 移出 → SampleWindLocal 的 DownburstHorizontal 矢量
		// （方向=玩家相对中心的径向向外，真正"四周扩散"）；这里只留垂直下沉 w。
		// v2.0.25 — w 也乘形成动画 grow（由弱到强，与视觉同步）。
		// v2.0.40 — **符号修复**：原为 `w +=`（上升），与注释"强下沉"矛盾——下击暴流
		// 应该是下沉气流（w 负），玩家在下击暴流中心被往下压。改 `w -=`（下沉）。
		// v2.0.85 — 下暴多实例：遍历下沉累加（每实例独立中心/强度/相位）。
		for (int di = 0; di < downbursts.Count; di++)
		{
			FxInst fx = downbursts[di];
			if (fx.strength <= 0.05)
		...
		// v2.0.2/v2.0.16 — 附属龙卷：极窄强旋转涡旋（叠加在宿主中心，漏斗状垂直结构）。
		// v2.0.85 — 多实例遍历：每龙卷独立中心/强度/相位累加上升吸力。
		for (int ti = 0; ti < tornadoes.Count; ti++)
		{
			FxInst fx = tornadoes[ti];
			if (fx.strength <= 0.05)
		...
		// v2.0.68 — 回退远古版风力轮廓（v1.0.1 反编译确认）。
		// v2.0.69 — 补齐远古版缺失项（阵风/drift/outflow）。
		// v2.0.76 — 对称眼壁双峰（用户明确要的分布：弱-较弱-中-较强-强(眼壁)-弱(风眼)-强(眼壁)
		// -较强-中-较弱-弱 —— 真实台风结构！v2.0.72-73 的非对称单峰理解错了，改回对称）：
		// sR=0 风眼弱 0.2、|sR|=0.5 中 0.5、|sR|=1 双侧眼壁峰 1.0、|sR|>1 外围衰减到 0.2。
		double sR = sWind / Rmax;   // 偏移功能保留（默认 off=0 即对称双峰）
		double aR = Math.Abs(sR);
		// v2.0.83 — 风圈概念（用户：搞 7/10/12 级风圈，7 级往外线性过渡至边缘 + 突发性
		...
		// v2.0.97 — 台风眼壁峰对齐 TyphoonEyeR（用户：台风眼存在于每个等级 + 眼壁不猛）：
		...
		double circle7 = cC7;   // v2.1.1 — 缓存（RefreshWindCircles 已刷新）
		...
		// v2.0.84 — 风圈保底（用户确认：max 下限兜底 + 跳过风眼）：风眼外每个半径至少
		...
		// v2.0.97 — ①跳过边界 0.7 → eyeRT（与渲染眼壁统一，TD/TS 无眼不跳过）；
		...
		{
			double c7b = cC7b;   // v2.1.1 — 缓存（RefreshWindCircles 已刷新）
			double c10 = cC10;
			double c12 = cC12;
			if (c7b > 0.25)
			{
		...
		if (spec.rotate > 0.05)
		{
			// v2.0.71 — 旋转分量不再中心爆炸（用户：方形框任何边缘都没有最大风力，
			...
			u += vmaxDisplay * spec.rotate * rotateScale * 0.35 * rot * edge * hDecay * num4 * intensity * torMask;   // v2.2.0 巨行星反气旋 rotateScale
		}

		u += vmaxDisplay * uBase * intensity * torMask;

		// v2.0.69 — 远古版（v1.0.0）补回项：
		// ① 风暴移动叠加风：风暴漂移（drift）本身也是风，叠加到切向 u（低层最强，ro≈1 峰值）。
		// v2.2.8 — includeDrift=false（粒子相对风）时跳过该项：原 Gauss 调制（中心 0.675×drift、
		...
		// v2.0.83 — 突发性大风（用户：7 级往外线性过渡 + 突发性高一两级的大风）：
		...
		// v2.0.94 — 严格限于风暴内部（用户：突发大风机制必须严格限风暴内部、判定线边缘
		// 向中心部近 100m）：原上限 Router(9Rmax) 太宽（风暴外也触发）→ 改为风暴判定边缘
		// Rmax×2.5（与 v2.0.93 主风场 edge 基准一致）再向中心内缩 100m——突发风只在
		...
		// ② PerlinNoise 湍流阵风（v1.0.0 原样，gustAmplitude=0.35）：风场不呆板、有阵风起伏。
		// v2.0.95 — 严格限风暴内部（用户：跟突发大风一样的判定）：边缘判定 Rmax×2.5 向中心
		...
		if (gustAmplitude > 0.0001 && aR * Rmax < Rmax * 2.5 - 100.0)
		{
			...
			// v2.0.77 — 波动下波动低（用户：参考值附近上下波动、下波动幅度低——持续风稳、
		...
		// 垂直分量（上升为主；下击暴流系数为负）——v2.0.21 修复：
		// 原 `w = Wmax*...` 是赋值，把龙卷/下击暴流在 364/380 累加的 w 覆盖清零；
		// 且地面摩擦 `w *= Clamp01(h/40)` 在 h=0 时把一切 w 抹零 → 玩家在地面 w 恒 0。
		// 改：通用 w 单独算（摩擦只削通用上升气流，地面无上升合理），附属 w 保留累加。
		// v2.0.41 — 技术债修复：vertShape 的"眼内下沉+眼壁上升+外围下沉"是台风眼壁模型，
		// 却被所有类型统一复用 → 用户在所有天气系统看到疑似台风眼的区域（单体/超单中心
		// 反而是 -0.55×Wmax 的下沉）。修复：按类型分流——
		//   台风：眼壁模型（中心下沉 + 眼壁上升 + 外围下沉，保持）；
		//   对流型（单体/多单体/超单/飑线/MCS）：中心上升核心（ro=0 最强、单调向外衰减）
		//   + 外围弱下沉出流，无"眼"。
		// v2.0.40 的龙卷补回逻辑由类型分流天然取代（对流型本就无中心下沉槽，
		// 台风 tornadoHost=false 不会挂龙卷）。
		double vertShape;
		if (type == StormType.Typhoon)
		{
			vertShape = Math.Exp(0.0 - Pow2((ro - 1.0) / 0.42)) - 0.55 * Math.Exp(0.0 - Pow2(ro / 0.55)) - 0.22 * Math.Exp(0.0 - Pow2((ro - 2.8) / 1.2));
			// v2.0.75 — 垂直分量跟随水平非对称（用户：风的分布"强-弱-强"双峰不对——
